...
use ::libc;
use libc::{FILE, O_CREAT, O_EXCL, O_RDONLY, O_RDWR, O_SYNC, O_WRONLY};
...
const MDB_DSYNC: u32 = O_SYNC as u32;
pub const MDB_O_LOCKS: mdb_fopen_type =
    MDB_O_RDWR | (MDB_CLOEXEC as u32) | ((MDB_O_MASK + 1) & !(MDB_O_MASK as u32));
pub const MDB_O_MASK: mdb_fopen_type =
    MDB_O_RDWR | (MDB_CLOEXEC as u32) | MDB_O_RDONLY | MDB_O_META | MDB_O_COPY;
pub const MDB_O_COPY: mdb_fopen_type = (O_WRONLY | O_CREAT | O_EXCL | MDB_CLOEXEC) as u32;
pub const MDB_O_META: mdb_fopen_type = (O_WRONLY as u32) | MDB_DSYNC | (MDB_CLOEXEC as u32);
pub const MDB_O_RDWR: mdb_fopen_type = (O_RDWR | O_CREAT) as u32;
pub const MDB_O_RDONLY: mdb_fopen_type = O_RDONLY as u32;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub const MDB_CURRENT: libc::c_int = 64;
...
unsafe extern "C" fn mdb_sem_unlock(mut mutex: mdb_mutexref_t) {
    let mut sb = {
        let mut init = sembuf {
...
pub const MDB_TXN_RDONLY: libc::c_int = MDB_RDONLY;
...
pub const MDB_END_SLOT: libc::c_int = MDB_NOTLS;
            ...
            if !((*dp).mp_flags as libc::c_int & (P_LOOSE | P_KEEP) != 0) {
                ...
                match current_block {
                    ...
                    _ => {
                        ...
                        if rc != 0 {
                            current_block = 14951790309026303142;
                            break;
                        }
        ...
        's_69: loop {
            ...
            if mop_len > n2 {
                ...
                loop {
                    ...
                    if *mop.offset(i.wrapping_sub(n2) as isize) == pgno.wrapping_add(n2 as pgno_t) {
                        current_block = 3220905346181743937;
                        break 's_69;
                    }
                    i = i.wrapping_sub(1);
                    if !(i > n2) {
            ...
            if rc != 0 {
                if rc == MDB_NOTFOUND {
                    ...
                    current_block = 13335373453721619300;
                    break;
                }
            } else {
...
unsafe extern "C" fn mdb_page_touch(mut mc: *mut MDB_cursor) -> libc::c_int {
        ...
        == 0x10 as libc::c_int)
    {
        if (*txn).mt_flags & MDB_TXN_SPILLS as libc::c_uint != 0 {
            ...
            if rc != 0 {
                current_block = 11480756240085776224;
            } else if !np.is_null() {
                current_block = 6837212362850905805;
            } else {
...
pub unsafe extern "C" fn mdb_txn_begin(
    ...
    if !parent.is_null() {
        ...
        txn = (*env).me_txn0;
        current_block = 3360869677227872190;
    }
    match current_block {
        ...
        == 0x1 as libc::c_int as libc::c_uint)
    {
        ...
        if ((*txn).mt_parent).is_null() {
            ...
            if !((*env).me_txns).is_null() {
                mdb_sem_unlock(((*env).me_wmutex).as_mut_ptr());
            }
        } else {
...
unsafe extern "C" fn _mdb_txn_commit(mut txn: *mut MDB_txn) -> libc::c_int {
    ...
    if !((*txn).mt_child).is_null() {
        ...
        if rc != 0 {
            current_block = 7743746421140782336;
        } else {
    ...
    match current_block {
        11875828834189669668 => {
            ...
            if (*txn).mt_flags & 0x20000 as libc::c_int as libc::c_uint
                == 0x20000 as libc::c_int as libc::c_uint
            {
                current_block = 2521923435558747777;
...
unsafe extern "C" fn mdb_env_init_meta(
    ...
    *((q as *mut libc::c_char).offset(PAGEHDRSZ as libc::c_uint as isize) as *mut libc::c_void
    ...
    loop {
        ...
        if len == -(1 as libc::c_int) && errno::errno().0 == EINTR {
            continue;
        }
    ...
    if flags & MDB_WRITEMAP as libc::c_uint != 0 {
        ...
        if flags & (MDB_NOMETASYNC | MDB_NOSYNC) as libc::c_uint == 0 {
            ...
            if msync(ptr as *mut libc::c_void, meta_size as size_t, rc) != 0 {
                rc = errno::errno().0;
                current_block = 7105584658716087227;
            } else {
                current_block = 3723050550557440724;
            }
        } else {
            current_block = 3723050550557440724;
        }
        ...
        loop {
            ...
            if !((*tx2).mt_spill_pgs).is_null() {
                ...
                if x as MDB_ID <= *((*tx2).mt_spill_pgs).offset(0 as libc::c_int as isize)
                    && *((*tx2).mt_spill_pgs).offset(x as isize) == pn
                {
                    current_block = 14825144610948408501;
                    break;
                }
            }
            if (*dl.offset(0 as libc::c_int as isize)).mid != 0 {
                    ...
                    && (*dl.offset(x_0 as isize)).mid == pgno
                {
                    ...
                    current_block = 1065572579742400582;
                    break;
                }
            }
            level += 1;
            ...
            if flags & MDB_PS_LAST != 0 {
                ...
                if (*mc).mc_flags & C_INITIALIZED as libc::c_uint != 0 {
                    if (*mc).mc_ki[(*mc).mc_top as usize] as libc::c_int == i as libc::c_int {
                        ...
                        current_block_30 = 8951056507734726400;
                    } else {
...
unsafe extern "C" fn mdb_cursor_set(
    ...
    if (*mc).mc_flags & C_INITIALIZED as libc::c_uint != 0 {
        ...
        if rc == 0 as libc::c_int {
            ...
            current_block = 15667240868457969050;
        } else {
            if rc > 0 as libc::c_int {
                let mut i: libc::c_uint = 0;
                ...
                if nkeys > 1 as libc::c_int as libc::c_uint {
                    ...
                    if rc == 0 as libc::c_int {
                        ...
                        match current_block {
                            15667240868457969050 => {}
                            _ => {
                                rc = 0 as libc::c_int;
                                (*mc).mc_flags &= !C_EOF as libc::c_uint;
                                current_block = 7001876241774639188;
                            }
                        }
                    } else {
    ...
    match op as libc::c_uint {
        ...
        2 | 3 => {
            if data.is_null() {
                ...
                current_block = 16102460135127032918;
            }
        }
        15 | 16 | 17 => {
            current_block = 16102460135127032918;
        ...
        5 => {
            if data.is_null() || (*mc).mc_flags & C_INITIALIZED as libc::c_uint == 0 {
                    ...
                    || (*(*mc).mc_xcursor).mx_cursor.mc_flags & C_EOF as libc::c_uint != 0
                {
                    ...
                    current_block = 6815114608351392307;
                }
            }
        }
        10 => {
        ...
        1 => {
            ...
            current_block = 13039516171629027761;
        }
        6 => {
            rc = mdb_cursor_last(mc, key, data);
            current_block = 1765866445182206997;
...
unsafe extern "C" fn _mdb_cursor_put(
    ...
    if insert_key != 0 {
                ...
                > (*env).me_nodemax as libc::c_ulong
        {
            ...
            *fresh31 = (PAGEHDRSZ as libc::c_uint).wrapping_sub(if 0 as libc::c_int != 0 {
            ...
            current_block = 8385991964087547802;
        } else {
        ...
        == 0x20 as libc::c_int
    {
        ...
        current_block = 15224145710640707353;
    } else {
        current_block = 14628437569462925565;
    }
    loop {
        match current_block {
            17353026871531185123 => {
                rdata = data;
                current_block = 13205422686744999527;
            }
            8385991964087547802 => {
                ...
                if (*(*mc).mc_db).md_flags as libc::c_int & 0x4 as libc::c_int == 0x4 as libc::c_int
                {
                    ...
                    if !((*leaf).mn_flags as libc::c_int & 0x4 as libc::c_int == 0x4 as libc::c_int)
                    {
                        ...
                        if flags == MDB_CURRENT as libc::c_uint {
                            current_block = 1352522448906803913;
                        } else {
                            dcmp = (*(*mc).mc_dbx).md_dcmp;
                            if (UINT_MAX as libc::c_ulong) < MDB_SIZE_MAX
                                && dcmp == Some(mdb_cmp_int as MDB_cmp_func)
                        ...
                        flags |= (F_DUPDATA | F_SUBDATA) as libc::c_uint;
                        current_block = 2114727675326044840;
                    } else {
                        fp = olddata.mv_data as *mut MDB_page;
                        match flags {
                            64 => {
                                current_block = 13578984274737404481;
                            }
                            _ => {
                                if (*(*mc).mc_db).md_flags as libc::c_int & MDB_DUPFIXED == 0 {
        ...
        match current_block {
            2114727675326044840 => {
                ...
                if dkey.mv_size != 0 {
                    ...
                    if rc != 0 {
                        current_block = 9517132498726757289;
                        break;
                    }
                    dkey.mv_size = 0 as libc::c_int as size_t;
                }
        ...
        == 0x20 as libc::c_int)
    {
        ...
        if (*leaf).mn_flags as libc::c_int & 0x4 as libc::c_int == 0x4 as libc::c_int {
            ...
            if (*leaf).mn_flags as libc::c_int & F_SUBDATA != 0 {
                ...
                if rc != 0 {
                    current_block = 11486415499138213681;
                } else {
            ...
            3689906465960840878 => {
                if (*leaf).mn_flags as libc::c_int & 0x1 as libc::c_int == 0x1 as libc::c_int {
                    ...
                    if rc != 0 || {
                        ...
                        current_block = 6370846400166877146;
                    }
                } else {
                    current_block = 6370846400166877146;
                }
        ...
        == 0x2 as libc::c_int
    {
            ...
            > (*(*(*mc).mc_txn).mt_env).me_nodemax as size_t
        {
            ...
            if node_size as ssize_t > room {
                ...
                current_block = 7982445986373296137;
            }
        } else {
                            ...
                            >= (*mc).mc_ki[(*mc).mc_top as usize] as libc::c_int
                        {
                            if (*m3).mc_ki[(*mc).mc_top as usize] as libc::c_uint >= nkeys {
                                ...
                                if rc == MDB_NOTFOUND {
                                    ...
                                    if rc != 0 {
                                        current_block = 14721846580494147640;
                                        break;
                                    }
...
unsafe extern "C" fn mdb_page_split(
    ...
    if ((*mc).mc_top as libc::c_int) < 1 as libc::c_int {
        ...
        if rc != 0 {
            current_block = 9775136079219326491;
        } else {
                ...
                == 0x2 as libc::c_int
            {
                ...
                while i < n {
                    ...
                    if (*ni).mn_flags as libc::c_int & F_BIGDATA != 0 {
                        ...
                        rc = mdb_page_get(mc, pg, &mut omp, NULL as *mut libc::c_int);
                        if rc != 0 as libc::c_int {
                            current_block = 2527059873129580647;
                            break 's_29;
                        }
                ...
                if subs == 0 && (*(*mc).mc_db).md_overflow_pages == 0 {
                    current_block = 3546813681910418346;
                } else {
